...
export default class Dashboard extends Component {
  ...
  selectList = (selectedList) => {
    ...
    if (
      ...
      this.state.selectedList.id === selectedList.id
    ) {
    } else {
      this.setState((prevState, props) => {
        return {
  ...
  selectTask = (task) => {
    ...
    Axios.get(
      `${API_URL}/profile/${this.state.selectedList.id}/${task.id}`,
      config
    )
      .then((response) => {
  ...
  deleteList = (singleListToDelete) => {
    let updatedListAfterDeletion = this.state.list.filter(
      (singleList) => singleList.id !== singleListToDelete.id
    );
    ...
    if (
      this.state.selectedList &&
      this.state.selectedList.id === singleListToDelete.id
    ) {
      this.setState((prevState, props) => {
  ...
  updateDeletedTaskList = ({ updatedListAfterDeletion, taskToDelete }) => {
    ...
    if (taskToDelete) {
      if (
        ...
        taskToDelete.id === this.state.selectedTask.id
      ) {
        this.setState((prevState, props) => {
  ...
  updateDeletedSubtask = ({ deleteUpdatedSubtasks, subtasktoDelete }) => {
    Axios.post(
      ...
      .then((response) => {
        ...
        this.updateDeletedTaskList({
          updatedListAfterDeletion: updatedSelectedTask,
        });
      })
      .catch((er) => {
  ...
  updateNotes = (newNote) => {
    ...
    this.editTaskName({
      oldTask: this.state.selectedTask,
      newTaskName: this.state.selectedTask.taskName,
    });
  };
